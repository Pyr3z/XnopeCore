...
namespace Xnope
{
    internal static class Debug
    {
        ...
        {
            Log.Warning("[XnopeCore] Spawning torches in triangle.");
        ...
        public static void TestRandomCellsInTriangleDraw(Map map, Rot4 dir)
        {
            Log.Warning("[XnopeCore] Spawning torches in random triangle cells.");

            var centre = map.Center;
            var targ = map.Center + dir.ToIntVec3() * 40;
            var halfAngle = 40;
            var magn = 20;

            foreach (var cell in CellsUtil.TriangleSide(centre, targ, halfAngle, magn))
            {
                if (!cell.GetThingList(map).Any(t => t.def == ThingDefOf.TorchLamp))
                {
                    var thing = ThingMaker.MakeThing(ThingDefOf.TorchLamp);
            ...
            foreach (var cell in CellsUtil.RandomCellsInTriangleFast(centre, targ, halfAngle, magn, 50))
            {
                if (!cell.GetThingList(map).Any(t => t.def == ThingDefOf.TorchLamp))
                {
                    var thing = ThingMaker.MakeThing(ThingDefOf.TorchLamp);
        ...
        public static void TestRandomCellsInTriangleDraw(Map map)
        {
            var centre = map.Center;
            var targ = CellRect.WholeMap(map).RandomCell;
            ...
            Log.Warning("[XnopeCore] Spawning torches in random triangle cells. halfAngle=" + halfAngle);
            ...
            foreach (var cell in CellsUtil.RandomCellsInTriangleFast(centre, targ, halfAngle, magn, 10))
            {
                if (!cell.GetThingList(map).Any(t => t.def == ThingDefOf.Snowman))
                {
